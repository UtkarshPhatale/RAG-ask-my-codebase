#!/usr/bin/env python3
"""
ULTIMATE Attention U-Net (32 Channels) - Clean Re-run
=====================================================
This is a re-run of the best model to save proper results.

Model: Improved Attention U-Net
Base Channels: 32
All optimizations: Deep supervision, Combo loss, Warmup+Cosine, AMP, etc.

Save location: models/checkpoints/v2/ultimate_32ch_rerun/
"""
...
print("="*80)
print("ULTIMATE ATTENTION U-NET (32 CHANNELS) - CLEAN RE-RUN")
print("="*80)
...
def validate_epoch(model, val_loader, criterion, device, metrics_calc):
    ...
    # Use simple loss for validation
    ...
    with torch.no_grad():
        for images, masks in val_loader:
            ...
            with autocast():
                ...
                loss = simple_criterion(main_output, masks)
...
def main():
    ...
    print(f"  Gradient clip:    {config['clip_grad']}")
    print(f"  Device:           {config['device']}")
    
    # Create directories with proper naming
    print("\n[2/8] Creating output directories...")
    checkpoint_dir = Path('../../models/checkpoints/v2/ultimate_32ch_rerun')
    log_dir = Path('../../models/logs/v2/ultimate_32ch_rerun')
    ...
    print(f"  ✓ Model: Improved Attention U-Net (32 channels)")
    ...
    print(f"  ✓ Loss:      {config['loss_type']} (with deep supervision)")
    print(f"  ✓ Optimizer: AdamW (lr={config['base_lr']}, wd=1e-4)")
    ...
    for epoch in range(1, config['num_epochs'] + 1):
        ...
        history['val_wt_dice'].append(float(val_wt_dice))
        history['val_tc_dice'].append(float(val_tc_dice))
        history['val_et_dice'].append(float(val_et_dice))
        ...
        if val_wt_dice > best_wt_dice:
            ...
            checkpoint = {
                ...
                'best_wt_dice': float(best_wt_dice),
                'val_wt_dice': float(val_wt_dice),
                'val_tc_dice': float(val_tc_dice),
                'val_et_dice': float(val_et_dice),
                'config': config,
                'history': history
            }
    ...
    print(f"  Final WT Dice:   {val_wt_dice:.4f}")
    ...
    summary = {
        ...
        'best_val_wt_dice': float(best_wt_dice),
        'final_val_wt_dice': float(val_wt_dice),
        'final_val_tc_dice': float(val_tc_dice),
        'final_val_et_dice': float(val_et_dice),
    ...
    print(f"  ✓ History saved:  {checkpoint_dir / 'training_history.json'}")
    print(f"  ✓ Summary saved:  {checkpoint_dir / 'training_summary.json'}")
